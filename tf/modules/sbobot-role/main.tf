resource "aws_iam_role" "sbobot_automation" {
  name        = var.role_name
  description = "Deploy sbo-bot code from GHA."
      ...
      [
        {
          ...
          Condition = {
            StringLike = {
              "token.actions.githubusercontent.com:sub" = "repo:aclemons/sbo-bot:*"
            }
            StringEquals = {
...
resource "aws_iam_role_policy" "iam_permissions" {
  ...
  role = aws_iam_role.sbobot_automation.id
